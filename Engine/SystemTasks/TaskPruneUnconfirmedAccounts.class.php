...
*
*/
class TaskPruneUnconfirmedAccounts extends BaseSystemTask
{
	...
	*/
	protected function executeTask()
	{
		...
		$this->core->notifyMediator("DatabaseManager", "executeQuery", 1, "UPDATE `account` SET `deleted`=1 WHERE `confirmed`=0 AND `lastonline` <= DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL ".$this->data['duration'].")");
		$counter = $this->core->notifyMediator("DatabaseManager", "getAffectedRows", 1);
		Tools::log("[TaskManager] Pruned <".$counter."> unconfirmed accounts.");
