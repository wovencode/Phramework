...
*
*/
class TaskPruneDeletedAccounts extends BaseSystemTask
{
	...
	*/
	protected function executeTask()
	{
		...
		$this->core->notifyMediator("DatabaseManager", "executeQuery", 1, "DELETE FROM `account` WHERE `deleted`=1 AND `lastonline` <= DATE_SUB(CURRENT_TIMESTAMP(), INTERVAL ".$this->data['duration'].")");
		$counter = $this->core->notifyMediator("DatabaseManager", "getAffectedRows", 1);
		Tools::log("[TaskManager] Pruned <".$counter."> deleted accounts.");
